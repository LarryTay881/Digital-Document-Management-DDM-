@extends('layouts.master')
@section('content')
{!! Toastr::message() !!}
<div class="page-wrapper">
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row">
                <div class="col-sm-12">
                    <h3 class="page-title">{{ __('messages.Form_List') }}</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ __('messages.Dashboard') }}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('/') }}">{{ __('messages.Forms') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('messages.Form_List') }}</li>
                    </ul>
                </div>
            </div>
        </div>

        <div class="col-12" style="text-align: right">
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#reportOptionsModal">{{ __('messages.Generate_Report') }}</button>
        </div>

        <!-- Report Options Modal -->
        <div class="modal custom-modal fade" id="reportOptionsModal" role="dialog">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-body">
                        <div class="form-header">
                            <h3>{{ __('messages.Select_Report_Format') }}</h3>
                        </div>
                        <div class="row">
                            <div class="col-6 text-right">
                                <a href="{{ route('form/list', ['format' => 'pdf']) }}" class="btn btn-primary">{{ __('messages.Generate_PDF') }}</a>
                            </div>
                            <div class="col-6 text-left">
                                <a href="{{ route('form/list', ['format' => 'excel']) }}" class="btn btn-primary">{{ __('messages.Generate_Excel') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive">
                    <table class="table table-striped" id="table1">
                        <thead>
                            <tr>
                                <th>{{ __('messages.Id') }}</th>
                                <th>{{ __('messages.Forms') }}</th>
                                <th>{{ __('messages.Upload_By') }}</th>
                                <th>{{ __('messages.Date_Time') }}</th>
                                <th class="text-center">{{ __('messages.Actions') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                        @foreach ($dataTemplate as $key=>$items )
                            <tr>
                                <td class="id">{{ $items->id }}</td>
                                <td class="id">
                                    @if($items->template_name != "")
                                        {{ $items->template_name }}
                                    @else
                                        Template {{ $items->id }}
                                    @endif                                  
                                </td>
                                <td><strong>{{ $items->upload_by }}</strong></td>
                                <td>{{ $items->date_time }}</td>
                                <td class="text-center">
                                    <a href="{{ route('form/response', ['id' => $items->id]) }}">
                                        <span class="badge bg-warning"><i class="bi bi-card-list"></i></span>
                                    </a>                                   
                                </td>                        
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>

@endsection